...
export function CalendarScreen({ sessions, currentUser, partnerUser }: Props) {
  ...
  sessions.forEach((s) => {
    if (s.userId === currentUser.username || s.userId === partnerUser.username) {
  ...
  const monthSessions = sessions.filter((s) => {
    ...
    return historyFilter === 'self' ? s.userId === currentUser.username : s.userId === partnerUser.username;
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {monthSessions.map((session) => {
              ...
              const durationMin = session.checkOutTime
                ...
                : null;
              const isSelfSession = session.userId === currentUser.username;
