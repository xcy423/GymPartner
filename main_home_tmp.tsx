...
export function Home({
  ...
  const heroMsg =
    weekSessions >= 5
      ? 'Incredible week! 5/5 sessions! G¡É'
      : weekSessions >= 3
      ? 'Weekly goal hit! =ƒÄ» Keep going for bonus!'
      : weekSessions === 2
      ? '1 more session to go! =ƒÆ¬'
      : weekSessions === 1
      ? '2 more sessions to go! =ƒÆ¬'
      : "Let's get moving! =ƒÆ¬ 0 sessions this week.";
      ...
      <div style={{ paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <AnimatePresence>
          {isPartner && (
            ...
            >
              =ƒæÇ Viewing {viewUser.displayName}'s progress
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {!sessionToday && !activeSession && !isPartner && (
          <div style={{
            ...
            <span style={{ fontSize: '16px', flexShrink: 0 }}>GÜán+Å</span>
            <div>
              <div style={{ fontSize: '13px', fontWeight: 700, color: C.warn }}>Hey, are you tracking? =ƒæï</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          {[
            ...
            { label: 'Multiplier', value: `+ù${viewUser.multiplier.toFixed(1)}`, color: C.primary },
        ...
        <div style={{
          ...
          <div style={{ fontSize: '15px', fontWeight: 800, color: C.textPrimary, marginBottom: '8px', letterSpacing: '-0.02em' }}>
            =ƒöÑ How streak multiplier works
          </div>
          <div style={{ fontSize: '13px', color: C.textMuted, lineHeight: 1.55, marginBottom: '12px' }}>
            Complete 3+ sessions every week to grow your multiplier. Each successful week = ++ù0.2 bonus, capped at +ù3.0. Miss a week and it resets to +ù1.0 GÇö but your points stay!
          </div>
          {/* Multiplier ladder */}
        ...
        {/* Session action area GÇö hidden in partner view */}
        {!isPartner && (
          <div>
            {/* Section header */}
              ...
              <div style={{
                  ...
                  <>
                    ...
                    >
                      {checkInPreview ? (
                        ...
                        <div style={{ textAlign: 'center', color: C.textFaint, fontSize: '13px' }}>=ƒô+ Upload check-in</div>
                      )}
                    </div>
                ...
                {activeSession && (
                  <div style={{ fontSize: '12px', color: C.primary, fontWeight: 600, textAlign: 'center' }}>
                    G£ô Checked in at {new Date(activeSession.checkInTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                ...
                >
                  ...
                  >
                    {checkOutPreview ? (
                      ...
                      <div style={{ textAlign: 'center', color: C.textFaint, fontSize: '13px' }}>=ƒô+ Upload check-out</div>
                    )}
                  </div>
